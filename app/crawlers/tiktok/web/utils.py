...
from app.crawlers.utils.logger import logger
from app.crawlers.douyin.web.xbogus import XBogus as XB
from app.crawlers.utils.utils import (
    gen_random_str,
    get_timestamp,
    extract_valid_urls,
    split_filename,
)
from app.crawlers.utils.api_exceptions import (
    APIError,
    APIConnectionError,
    APIResponseError,
...
class TokenManager:
    ...
    def gen_real_msToken(cls) -> str:
        ...
        # 修复httpx 0.28版本代理参数问题 - 使用mounts替代proxies
        client_kwargs = {"transport": transport}
        if cls.proxies:
            # 为HTTP和HTTPS配置代理mounts
            proxy_mounts = {
                "http://": httpx.HTTPTransport(proxy=cls.proxies.get("http", cls.proxies.get("https"))),
                "https://": httpx.HTTPTransport(proxy=cls.proxies.get("https", cls.proxies.get("http")))
            }
...
class WebCastIdFetcher:
    """直播ID获取器 - 从TikTok直播URL中提取webcast_id"""
    
    # 预编译正则表达式
    _TIKTOK_LIVE_URL_PATTERN = re.compile(r"live/([^/?]*)")
    _TIKTOK_LIVE_URL_PATTERN2 = re.compile(r"http[s]?://live.tiktok.com/(\d+)")
    _TIKTOK_LIVE_URL_PATTERN3 = re.compile(r"reflow/([^/?]*)")
    ...
    async def get_webcast_id(cls, url: str) -> str:
        ...
        try:
            ...
            async with httpx.AsyncClient(
                ...
                live_pattern = cls._TIKTOK_LIVE_URL_PATTERN
                live_pattern2 = cls._TIKTOK_LIVE_URL_PATTERN2
                live_pattern3 = cls._TIKTOK_LIVE_URL_PATTERN3
            ...
            # 捕获所有与 httpx 请求相关的异常情况
            raise APIConnectionError("请求端点失败，请检查当前网络环境。 链接：{0}，异常类名：{1}，异常详细信息：{2}"
    ...
    async def get_all_webcast_id(cls, urls: list) -> list:
        """
        获取直播webcast_id,传入列表url都可以解析出webcast_id

        Args:
            urls: list: 列表url

        Return:
            webcast_ids: list: 直播的唯一标识，返回列表
        """
        ...
        webcast_ids = []
        for url in urls:
            webcast_id = await cls.get_webcast_id(url)
            webcast_ids.append(webcast_id)
        return webcast_ids
